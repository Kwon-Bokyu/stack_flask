...
class Item:
    ...
    def __str__(self):
        return "%s [%s]" % (self.partno, self.position)
        ''' '''
        ...
        for i in range(0, len(rotate)):
            ...
            if fit:
                ...
                if self.fix_point == True :
                    ...
                    # print(f">> [시작]item {item.partno} on ({x}, {y}, {z})")  
                    for i in range(3):
                        # print(f">> 조정 {i} times")  
                        # fix height
                        y = self.checkHeight([x,x+float(w),y,y+float(h),z,z+float(d)])
                        # print(f">> [{i}]item {item.partno} on ({x}, {y}, {z})")  
                        # fix width
...
class Packer:
    ...
    def printStatus(self):
        ...
        for box in self.bins:
            ...
            for item in box.items:
                tot_cnt += 1
                tot_volume += float(item.width) * float(item.height) * float(item.depth)
                tot_weight += item.weight
            
            print('    사용부피 : {}%'.format(round(tot_volume / float(volume) * 100 ,2)))
            print('    남은부피 : ', float(volume) - tot_volume )
            print('    적재아이템수 : {} 개'.format(tot_cnt))
            print('    적재아이템중량 : {} kg'.format(tot_weight))
            print("    gravity distribution : ",self.gravityCenter(box))

            painter = Painter(box)
            # 전체 모습(최종 적재 상태) 이미지 생성
            fig = painter.plotBoxAndItems(
                title=f"{box.partno} - Full View",
                alpha=0.8,
                write_num=False,
                fontsize=10
            )
    ...
    def putOrder(self):
        ...
        for bin in self.bins:
            bin.items.sort(key=lambda item: (item.position[0], item.position[1], item.position[2]), reverse=False)
    ...
    # distribute_items: 불필요 True 고정
    # 컨테이너의 수량이 제한된 상태에서 최적 적재 계획을 산출하기 위한 함수
    def pack(self, bigger_first=False,distribute_items=True,fix_point=True,check_stable=True,support_surface_ratio=0.75,binding=[],number_of_decimals=DEFAULT_NUMBER_OF_DECIMALS):
        ...
        for idx,bin in enumerate(self.bins):
            for no,item in enumerate(self.items):
    ...
    def sort(self):
        print("######### BEFORE SORT ###########")
        print("  NAME    LVL   LDBR   VOLUME")
        for i in self.items:
            print(f"{i.name}  {i.level}  {i.loadbear}  {i.getVolume()} ,")
        
        self.items.sort(key=lambda item: (-item.level, item.loadbear, item.getVolume()), reverse=True)
        print("######### AFTER SORT ###########")
        print("  NAME    LVL   LDBR   VOLUME")
        for i in self.items:
            print(f"{i.name}  {i.level}  {i.loadbear}  {i.getVolume()} ,")

    # 최소한의 컨테이너를 사용할 수 있는 최적 적재 계획을 산출하기 위한 함수
    # item 이 선택된 컨테이너에 적재될 수 없는 경우. 새로운 컨테이너를 생성하여 적재시도함.
    def calculate(self, bigger_first=False,distribute_items=True,fix_point=True,check_stable=True,support_surface_ratio=0.75,binding=[],number_of_decimals=DEFAULT_NUMBER_OF_DECIMALS):
        ...
        bin_idx = 0
        unfitted = []
        #print(f"{len(self.items)} items has not packed yet")
        while self.items:
            last_fitted = -1
            for no, item in enumerate(self.items):
                bin : Bin = self.bins[bin_idx]
                self.pack2Bin(bin, item, fix_point, check_stable, support_surface_ratio)
                
                unfitted = bin.unfitted_items
                bin.unfitted_items = []
                if unfitted:
                    break
                last_fitted = no
            ...
            self.items = self.items[last_fitted + 1:]     # (문제) item이 [unfit, fit, unfit, ]과 같이 적재 가능 여부가 불규칙한 경우 슬라이싱 하는 방식이 비효율적일 수 있음. 
            if not self.items :
                break
            #print(f"item {item.partno} is unfitted to bin")

            # (문제) 남은 아이템을 적재할 수 있는 bin 종류가 없으면 self.unfitted에 넣고 items에서 제거 > 또는 오류 처리 ?
            if unfitted and (item.getVolume() > bin.getVolume() or (item.width > bin.width or item.height > bin.height or item.depth > bin.depth)):
                self.unfit_items.append(item)
                self.items.remove(item)
            elif unfitted and bin_idx == len(self.bins) - 1:
                new_bin : Bin = copy.deepcopy(self.bins[bin_idx])
                new_bin.clearBin()
                new_bin.partno = new_bin.partno + '_1'
        ...
        for idx, bin in enumerate(self.bins):
            self.bins[bin_idx].gravity = self.gravityCenter(bin)
        # put order of items
        self.putOrder()
...
class Painter:
    ...
    def plotBoxAndItems(self,title="",alpha=0.2,write_num=False,fontsize=10):
        ...
        axGlob.set_xlim([0, float(self.width)*0.8])
        axGlob.set_ylim([0, float(self.depth)*0.8])
        axGlob.set_zlim([0, float(self.height)*0.8])
        axGlob.set_axis_off()
        return plt


    def plotBoxAndItemsStepByStep(self, title="", alpha=0.2, write_num=False, fontsize=10):
        """ 
        화물이 단계적으로 적재되는 모습을 보여주는 함수 
        """
        # 화물을 width, depth, height 순으로 정렬
        # sorted_items = sorted(self.items, key=lambda item: (item.position[2], item.position[1], item.position[0]))
        ...
        for item in self.items:
            if item.partno == last_partno:
                current_group.append(item)
            else:
                if current_group:
                    grouped_items.append(current_group)
                current_group = [item]
            last_partno = item.partno
        ...
        # plot bin
        self._plotCube(axGlob, 0, 0, 0, float(self.width), float(self.height), float(self.depth), color='black', mode=1, linewidth=6, text="")
        ...
        for group in grouped_items:
            ...
            # 각 그룹이 적재된 후 단계별로 이미지 생성
            step_fig = copy.deepcopy(fig)
            steps.append(step_fig)
            plt.close(fig)
        
        plt.title(title)
        axGlob.set_box_aspect([self.width, self.height, self.depth])
        plt.subplots_adjust(left=0, right=0.1, top=1, bottom=0)
        plt.tight_layout()
        self.setAxesEqual(axGlob)

        return steps    

    def plotBoxAndItemsByInterval(self, title="", alpha=1.0, write_num=False, fontsize=10, step_interval=5):
        """
        stpe interval 값에 따라 화물을 일정 간격으로 적재하는 이미지를 보여주는 함수
        """
        steps = []
        fig = plt.figure(figsize=(24,16))
        ...
        # 화물을 step_interval 간격으로 보여줌
        for step_num in range(0, len(self.items),step_interval):
            # 현재까지 적재된 화물만 보여줌
            for item in self.items[:step_num + step_interval]:
                rt = item.rotation_type
                x, y, z = item.position
                [w, h, d] = item.getDimension()
                color = item.color
